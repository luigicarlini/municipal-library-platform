...
-------------------------------------------------------------------------------
-- 0.b  assicura l’indice UNIQUE parziale su ISBN “attivi”
-------------------------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        WHERE n.nspname = 'public'
          AND c.relname = 'ux_books_isbn_active'
    ) THEN
        CREATE UNIQUE INDEX ux_books_isbn_active
               ON books(isbn)
               WHERE deleted = FALSE;
    END IF;
END $$;   --  ←  chiudi il blocco con ‘;’
...
ON CONFLICT (isbn) WHERE deleted = FALSE DO NOTHING;
