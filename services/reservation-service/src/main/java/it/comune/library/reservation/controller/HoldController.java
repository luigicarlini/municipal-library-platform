...
//import it.comune.library.reservation.service.LoanLifecycleService;
...
import io.swagger.v3.oas.annotations.headers.Header; //  ← nuovo 
...
public class HoldController {
        ...
        public ResponseEntity<List<HoldDto>> searchPaged(
                        ...
                        @RequestParam(required = false) String title,              //  ← 1 riga
         ...
         * 
         * ██████████████████████████████████████████████████████████████████
         * 8. Restituzione libro (loan lifecycle)
         * ██████████████████████████████████████████████████████████████████
         */
        @Operation(summary = "Registra la restituzione di un libro")
        @ApiResponses({
                        @ApiResponse(responseCode = "204", description = "Restituzione registrata"),
                        @ApiResponse(responseCode = "404", description = "Hold non trovata"),
                        @ApiResponse(responseCode = "409", description = "Hold non restituibile")
        })
        @PutMapping("/{id}/return")
        public ResponseEntity<Void> markReturned(
                        @Parameter(description = "ID della prenotazione") @PathVariable UUID id) {

                return holdRepository.findById(id)
                                .map(hold -> { // λ → ResponseEntity<Void>
                                        /* ammessa solo se READY o OVERDUE */
                                        if (hold.getStatus() != HoldStatus.READY &&
                                                        hold.getStatus() != HoldStatus.OVERDUE) {
                                                return ResponseEntity // 409 (con <Void>)
                                                                .status(409)
                                                                .<Void>build();
                                        }
                                        ...
                                        return ResponseEntity // 204 (con <Void>)
                                                        .noContent()
                                                        .<Void>build();
                                })
                                /* se la hold non esiste → 404 */
        ...
        /*
         * ██████████████████████████████████████████████████████████████████
         * 9. LEGACY - ricerca per titolo (no paging)
         * ██████████████████████████████████████████████████████████████████
         */
